...
def build_params(manifest_path: str, run_name: str, epochs: int, batch_size: int,
                 ...
                 valid_basin_split: str = "train",
                 trainer_test_basin_split: str = "train") -> Dict:
    """
    ...
    params = {
        ...
        "dataset_params": {
            ...
            "test_basin_split": trainer_test_basin_split,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Load a saved model state_dict and run diagnostics/evaluation without training",
    )
    ...
    parser.add_argument(
        "--lean-tuning", action="store_true",
        help="Build the trainer's unused post-fit test-loss loader from the validation basins "
             "instead of duplicating the full training fleet in memory. Training, early "
             "stopping, and forecast_report evaluation are unchanged.",
    )
    ...
    valid_basin_split = ("basin_valid" if args.basin_validation_count > 0
    ...
    params = build_params(training_manifest, args.name, args.epochs, args.batch_size,
                          ...
                          valid_basin_split=valid_basin_split,
                          trainer_test_basin_split=(valid_basin_split if args.lean_tuning
                                                    else "train"))
    ...
    if args.eval_only_checkpoint:
        state = torch.load(args.eval_only_checkpoint, map_location=model.device, weights_only=True)
        model.model.load_state_dict(state)
        print("evaluation-only checkpoint loaded from %s" % args.eval_only_checkpoint)
    else:
        if args.init_from:
