...
This directory holds Water's scripts for training and evaluating a single hybrid GR4/ODE
hydrology model across many catchments at once (`HybridGR4MultiBasin`), instead of the
one-model-per-basin `HybridGR4Forecast` setup. Models and generic execution infrastructure come
from Flow Forecast; this directory owns the hydrology cohorts, experiment settings, and scientific
comparisons. The pipeline has three stages:
...
## Setup and framework dependency

Install the sibling Flow Forecast checkout into the environment used to run Water:

```bash
python -m pip install -e ../flow-forecast
```

`FLOW_FORECAST_PYTHON` may point the shell launchers at a different Python executable. The Python
scripts import `flood_forecast` as a normal dependency and must not vendor or copy framework code.

## Reuse contract

New experiment work must use Flow Forecast's existing infrastructure before adding orchestration
here:

- training and W&B logging: `flood_forecast.trainer`, `time_model`, and `pytorch_training`;
- dated forecasting and metrics: `flood_forecast.evaluator` and `deployment.inference`;
- interactive forecast plots: `flood_forecast.plot_functions` and the Plotly logging in `trainer`;
- hyperparameter sweeps: Flow Forecast's sweep-aware config path and W&B sweep tooling.

The current split-aware evaluator, comparison gallery, and fixed ladder launchers predate this
boundary and are legacy hydrology adapters. Do not expand them into parallel framework features.
Where the multi-basin loader prevents direct use of a Flow Forecast API, generalize that API in a
Flow Forecast PR and reduce the Water adapter afterward. Fixed scientific ablations may keep thin
launch scripts, but should not implement another sweep engine.

## Data prerequisites

The manifest builder reads directly from this repository's ignored `pilot_data` tree:

- `pilot_data/scrapes/CO/<site_id>/<site_id>_static.json` — static
  basin attributes (drainage area, mean basin elevation, gauge altitude).
- `pilot_data/scrapes/CO/<site_id>/<site_id>_hourly_full.csv` — the
...
- `pilot_data/scrapes/CO/registry.json` — scrape status registry;
  only sites marked `"status": "completed"` are considered.
- `pilot_data/embedding_dataset/CO/embeddings_concat.pt` — the
  pretrained per-basin embedding bank (`{"site_ids": [...], "embeddings": Tensor}`).
- `pilot_data/snodas_series/CO/<site_id>_snodas_swe.csv` — optional
...
```json
{
  "embedding_path": "pilot_data/embedding_dataset/CO/embeddings_concat.pt",
...
Per-basin field notes: `csv_path` and (optional) `swe_csv_path` are absolute paths into this
repository's local data tree; `split` is `"train"` or `"holdout"`; `has_embedding` gates whether
