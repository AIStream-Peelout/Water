...
# Warm-start floors: the batched-Euler calibration tolerates parameters (X4 near 2 h, tiny X1)
# that are marginally stable or worse under the training-side hourly RK4 ODE — starting there
# produces systemic NaN losses. Calibration files stay unclamped; only the regression/inversion
# targets are floored into solver-safe territory.
WARM_START_FLOORS = {"X1": 25.0, "X4": 8.0}
...
def targets_to_fractions(calibrations: Dict[str, Dict], site_ids: List[str]) -> torch.Tensor:
    ...
    for site in site_ids:
        ...
        for name, floor in WARM_START_FLOORS.items():
            values[name] = max(values[name], floor)
...
def select_fit_sites(manifest: Dict, calibrations: Dict, bank_lookup: Dict[str, int],
                     fit_split: str = "train") -> Tuple[List[str], List[str]]:
    """Selects embedded and unembedded calibration sites from the training split only.

    Basin-validation and final-holdout calibrations must never reach the parameter head: doing so
    leaks the evaluation basin's fitted hydrology into its supposedly transferable initialization.
    """
    eligible = [
        basin["site_id"] for basin in manifest["basins"]
        if basin.get("split") == fit_split and basin["site_id"] in calibrations
    ]
    embedded = [site for site in eligible if site in bank_lookup]
    unembedded = [site for site in eligible if site not in bank_lookup]
...
def main() -> None:
    ...
    parser.add_argument("--fit-split", default="train",
                        help="Manifest split whose calibrations may supervise the head")
    ...
    embedded, unembedded = select_fit_sites(
        manifest, calibrations, bank_lookup, fit_split=args.fit_split)
